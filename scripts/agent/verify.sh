#!/bin/bash
# End-to-end verification: production build + banner generation
# Usage: bash scripts/agent/verify.sh [username ...]
...
# 3. Start dev server
if ! bash "$SCRIPTS_DIR/dev-start.sh"; then
...
for U in "${USERNAMES[@]}"; do
  if ! bash "$SCRIPTS_DIR/img-test.sh" "$U"; then
...
# 5. Stop dev server
bash "$SCRIPTS_DIR/dev-stop.sh" > /dev/null 2>&1 || true
